...
export const CustomText = styled.span<TextProps>`
	font-size: ${({ big, small }) =>
		big ? "32px" : small ? "12px" : "14px"}; // Tamaño de fuente basado en las props
	font-weight: ${({ big }) => (big ? "bold" : "normal")}; // Peso de la fuente para texto grande
	background-color: ${({ background }) =>
		background ||
		"transparent"}; // Color de fondo, predeterminado a transparente si no se proporciona
	color: ${({ color }) =>
		color || "#03050F"}; // Color del texto, predeterminado a negro si no se proporciona
